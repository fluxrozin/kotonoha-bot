...
2. スレッド名を設定（`topic` が指定されていない場合は、メッセージの最初の 100 文字を使用）
3. スレッド内で Bot が応答
...
| **スレッド型会話**   | スレッド内で会話を継続         | Phase 2  |
| **DM 会話**          | ダイレクトメッセージで会話     | Phase 2  |
| **聞き耳型会話**     | 自然に会話に参加               | Phase 2  |
| **会話履歴保持**     | 会話の文脈を保持               | Phase 1  |
| **会話履歴リセット** | 会話履歴をクリア               | Phase 2  |
...
| **セッション保存** | SQLite に永続化              | Phase 2  |
| **セッション復元** | ボット再起動時に復元         | Phase 2  |
| **セッション削除** | 非アクティブセッションを削除 | Phase 2  |

### 3.3 AI 機能

| 機能名                 | 説明                       | 実装状況 |
| ---------------------- | -------------------------- | -------- |
| **基本応答生成**       | Gemini API で応答を生成    | Phase 1  |
| **Flash/Pro 使い分け** | タスクに応じてモデルを選択 | Phase 1  |
| **プロンプト最適化**   | 場面緘黙支援向けプロンプト | Phase 1  |
| **判定機能**           | 聞き耳型の判定（Yes/No）   | Phase 2  |
| **フォールバック**     | API エラー時の代替処理     | Phase 3  |

### 3.4 エラーハンドリング機能

| 機能名               | 説明                             | 実装状況 |
| -------------------- | -------------------------------- | -------- |
| **API エラー処理**   | Gemini/Discord API エラー処理    | Phase 1  |
| **レート制限対応**   | レート制限の監視と対応           | Phase 2  |
| **リトライロジック** | エラー時の自動リトライ           | Phase 2  |
...
| 機能名             | 説明                           | 実装状況 |
| ------------------ | ------------------------------ | -------- |
| **ログ出力**       | 動作ログの記録                 | Phase 3  |
| **モニタリング**   | パフォーマンス監視             | Phase 3  |
| **バックアップ**   | データベースの自動バックアップ | Phase 3  |
| **ヘルスチェック** | システム状態の確認             | Phase 3  |
