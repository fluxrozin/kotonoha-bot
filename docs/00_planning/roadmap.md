...
| **Phase 11** | ✅ 完了   | 約1日    | ハイブリッド検索（pg_bigm）                           |
| **Phase 12** | ⏳ 未実装 | 2-3 日   | Reranking（オプション）                               |
| **Phase 13** | ⏳ 未実装 | 3-6 日   | 高度なモニタリング機能（管理者用ダッシュボード含む） |
| **Phase 14** | ⏳ 未実装 | 2-3 日   | コスト管理機能                                       |
| **Phase 15** | ⏳ 未実装 | 3-4 日   | 監査ログ機能                                         |
| **Phase 16** | ⏳ 未実装 | 2-3 日   | 設定管理機能                                         |
| **Phase 17** | ⏳ 未実装 | 2-3 日   | パフォーマンス分析                                   |
| **Phase 18** | ⏳ 未実装 | 1-2 日   | ログ出力の強化                                       |
| **Phase 19** | ⏳ 未実装 | 2-3 日   | モデル切り替えオプション                             |
| **Phase 20** | ⏳ 未実装 | 1-2 日   | バックアップの自動化強化                             |
| **Phase 21** | ⏳ 未実装 | 2-3 日   | データベース最適化                                   |
| **Phase 22** | ⏳ 未実装 | 2-3 日   | コスト計算機能                                       |
| **Phase 23** | ⏳ 未実装 | 15-20 日 | 統合知識ベース機能（会話・添付ファイル・ウェブ）   |
| **Phase 24** | ⏳ 未実装 | 10-15 日 | 会話の要約・ハイライト機能                           |
| **Phase 25** | ⏳ 未実装 | 8-12 日  | 議題提案機能                                         |
| **Phase 26** | ⏳ 未実装 | 5-10 日  | 聞き耳モードでの自動リアクション機能                 |
| **Phase 27** | ⏳ 未実装 | 5-8 日   | ポジティブフィードバック生成機能                     |
| **Phase 28** | ⏳ 未実装 | 5-8 日   | 翻訳要約機能                                         |
| **Phase 29** | ⏳ 未実装 | 20-30 日 | 音声チャンネル機能（要約・議事録、合成音声発話）     |
| **Phase 30** | ⏳ 未実装 | 15-20 日 | コンテキスト理解の向上機能                           |
...
**詳細**: [Phase 8 実装完了報告](./phases/phase08.md)
...
プロバイダーを切り替える必要がないため、LiteLLM の主な価値（統一インターフェース）が不要である。また、LiteLLM には以下のデメリットがある：
...
- **パフォーマンス向上**: オーバーヘッドの削減、メモリ使用量の削減
- **セキュリティ向上**: セキュリティリスクの削減
...
### Phase 11: ハイブリッド検索の実装 ✅ 完了

**目標**: pg_bigm を使用したハイブリッド検索を実装し、検索品質を向上させる
...
**詳細**: [Phase 11 実装完了報告](./phases/phase11.md)

---

### Phase 12: Reranking の実装（オプション）⏳ 未実装
...
### Phase 14: コスト管理機能 ⏳ 未実装

**目標**: トークン使用量とコストを追跡・管理する機能を実装する

**期間**: 約 2-3 日

**前提条件**: ✅ **Phase 9（LiteLLM 削除、Anthropic SDK 直接使用への移行）が完了していること**、
✅ **Phase 10（完全リファクタリング）が完了していること**（`anthropic_provider.py` の戻り値変更が完了しているため）
...
**前提条件**: ✅ **Phase 8（PostgreSQL への移行）が完了していること**
（pgvector が必要）、✅ **Phase 10（完全リファクタリング）が完了していること**
...
**前提条件**: ✅ **Phase 10（完全リファクタリング）が完了していること**、
✅ **Phase 23（統合知識ベース機能）が完了していること**、
✅ **Phase 24（会話の要約・ハイライト機能）が完了していること**
...
### Phase 26: 聞き耳モードでの自動リアクション機能 ⏳ 未実装
...
### Phase 29: 音声チャンネル機能（要約・議事録、合成音声発話）⏳ 未実装
...
**Phase 11**: ✅ 完了（リスクは解消済み）

- ✅ pg_bigm拡張のビルド・インストールの複雑さ → 解消済み（Dockerfile.postgresで実装）
- ✅ インデックス作成時のパフォーマンス影響（大量データの場合） → 解消済み（マイグレーションで適切に処理）
...
- ✅ **Phase 11**: 完了（ハイブリッド検索、pg_bigm）

次の実装ステップは **Phase 12（Reranking）** または **Phase 13（高度なモニタリング機能）** です。

### 6.1 推奨実装順序

1. ✅ **Phase 7（aiosqlite への移行）**: 完了（2026 年 1 月 15 日）

2. ✅ **Phase 8（PostgreSQL への移行）**: 完了（2026 年 1 月）
   - **重要**: ベクトル検索機能の実現（pgvector を使用）
   - **重要**: Phase 23-25 の知識ベース機能の前提条件
   - データベース抽象化レイヤーの確立
   - スケーラビリティの確保
...
5. ✅ **Phase 11（ハイブリッド検索）**: 完了（2026年1月20日）
   - ✅ 検索品質の向上
   - ✅ pg_bigm拡張の実装
...
7. **Phase 13-22**: Phase 10 完了後に実施（Phase 10 は完了済み）
   ...
   - **Phase 16-18**: 設定管理、パフォーマンス分析、ログ強化
   - **Phase 19-21**: 自動化・最適化機能
   - **Phase 22**: コスト計算機能

8. **Phase 23-28**: ロードマップ外の高優先度機能（Phase 8 で pgvector が必要）
   - **Phase 23**: 統合知識ベース機能（pgvector を使用）⭐⭐⭐⭐⭐
   - **Phase 24**: 会話の要約・ハイライト機能（知識ベース機能と同時に実装）⭐⭐⭐⭐⭐
   - **Phase 25**: 議題提案機能（知識ベース機能の後に実装）⭐⭐⭐⭐⭐
   - **Phase 26**: 聞き耳モードでの自動リアクション機能⭐⭐⭐⭐⭐
   - **Phase 27**: ポジティブフィードバック生成機能⭐⭐⭐⭐☆
   - **Phase 28**: 翻訳要約機能（多言語対応の強化）⭐⭐⭐⭐☆

9. **Phase 29-30**: ロードマップ外の低優先度機能（技術検討後）
   - **Phase 29**: 音声チャンネル機能（要約・議事録、合成音声発話）⭐⭐⭐☆☆
   - **Phase 30**: コンテキスト理解の向上機能（pgvector を使用）⭐⭐⭐☆☆
...
- [Phase 10 詳細実装計画](./phases/phase10-implementation-steps.md): 各ステップ（Step 0-7）の詳細な実装手順とコード例
- [Phase 10 テスト・完了基準・リスク管理](./phases/phase10-implementation-testing.md): テストコードリファクタリング、完了基準、リスク管理、改善提案
- [Phase 11 実装完了報告](./phases/phase11.md): ハイブリッド検索（pg_bigm）の実装完了報告

---

**作成日**: 2026 年 1 月 14 日  
**最終更新日**: 2026 年 1 月 20 日（Phase 11 の完了を反映）  
**バージョン**: 17.0  
**作成者**: kotonoha-bot 開発チーム

### 更新履歴

- **v17.0** (2026-01-20): Phase 11 の完了を反映、実装状況サマリーと詳細セクションを更新、リスク管理セクションを更新
