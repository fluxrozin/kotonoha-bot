...
| 重複コード | 6箇所以上 | 0箇所 |
| 設定管理 | main.py にログ設定混在 | config.py で Pydantic V2 化（移動なし） |
...
- `config.py` がすべての設定を管理（Pydantic V2 化で改善）
...
Phase 10 は全体をリファクタリングするため、リファクタリング対象のコードにテストがない場合は、まずテストを書く必要がある。
...
- **各ステップ完了時にテストを実行**: Step 0-7 の各ステップ完了時に必ず全テストを実行する
...
1. **レイヤーの簡略化**: `bot/`, `services/`, `db/`, `errors/`, `utils/`, `rate_limit/` の構造（既存の `features/` と `external/` は維持）
2. **handlers.py の分割**: 832 行 → `bot/handlers/` パッケージ内で `mention.py`, `thread.py`, `eavesdrop.py` に分割（必須）
3. **設定管理の整理**: `config.py` はそのまま維持、Pydantic V2 化を実施（移動なし）
4. **エラーハンドリングの統一**: `errors/` はそのまま維持、`errors/messages.py` を新規作成してメッセージを一元管理
5. **重複コードの削除**: 日付フォーマット、プロンプト読み込み、エラーメッセージを統一
6. **router/ の統合**: `router/message_router.py` → `bot/router.py` に移動
7. **commands/ の統合**: `commands/chat.py` → `bot/commands.py` に移動
...
| **handlers.py 分割**   | 1 ファイル (832 行)  | 4 ファイル（分割）        |
| **重複コード削除**     | 6+ 箇所             | 0 箇所                    |
...
**各ステップの詳細な実装手順、コード例、コーディング規約は [詳細実装計画](./phase10-implementation-steps.md) を参照してください。**
...
**詳細なチェックリストと品質チェックコマンドは [テスト・完了基準・リスク管理](./phase10-implementation-testing.md#8-完了基準とチェックリスト) を参照してください。**
...
1. **Git ブランチ戦略**
   - `feature/phase10-refactoring` ブランチで作業
   - 各 Step 完了時にコミット
   - 問題発生時は直前のコミットに戻す
...
**詳細なリスク管理、ロールバック戦略、テスト戦略は [テスト・完了基準・リスク管理](./phase10-implementation-testing.md#9-リスク管理) を参照してください。**

---

## 10. 詳細実装計画へのリンク

このドキュメントは Phase 10 の基本方針と概要を説明しています。
...
1. **[実装ガイド（メイン）](./phase10-implementation.md)**: コーディング規約、リファクタリング方針、新フォルダ構造
2. **[詳細実装計画](./phase10-implementation-steps.md)**: 各ステップ（Step 0-7）の詳細な実装手順とコード例
3. **[テスト・完了基準・リスク管理](./phase10-implementation-testing.md)**: テストコードリファクタリング、完了基準とチェックリスト、リスク管理、改善提案、付録
...
**詳細**: [詳細実装計画 - Step 3](./phase10-implementation-steps.md#65-step-3-aiサービスと抽象化---servicesaipy-の統合例外ラッピング戻り値変更day-4-52日)
...
1. **[実装ガイド（メイン）](./phase10-implementation.md) を確認する**
2. **[詳細実装計画](./phase10-implementation-steps.md) を確認する**
...
- v2.0 (2026-01-19): 基本方針と詳細実装を分離、ゼロベースで再整理、完了報告追加
- v1.0 (2026-01-19): Phase 10 として初版作成
