...
- [x] 依存関係グラフが生成されている（`/tmp/deps_before.svg` が生成済み）
- [x] 循環依存が検出されていない（特に `services/ai.py` と `services/session.py` の相互依存がないことを確認）
  - `pydeps --show-cycles` で循環依存は検出されていない
  - `services/ai.py` と `services/session.py` の相互依存は確認済み（なし）
- [x] 依存関係の方向性ルールが文書化されている（本ドキュメントの「2. 依存関係の方向性ルールを文書化」セクションに記載）
- [ ] 設計レビューが完了している
- [x] 循環参照のリスクがある箇所が特定され、対策が決定されている（`phase10-implementation-testing.md` の「9.5 循環参照（循環インポート）のリスク」セクションに記載）
- [x] テスト汚染コードの検出が完了し、解消計画が作成されている
  - 検出結果: `logger.debug()` のみ（テスト汚染コードではない）
  - 解消不要（正常なログ出力）
- [x] **不要な依存関係の特定が完了している（deptry を使用）**
  - `uv run deptry .` を実行し、問題なし（Success! No dependency issues found.）
- [x] **`psycopg2-binary` の削除検討が完了している（asyncpg のみで動作することを確認）**
  - `pyproject.toml` に `psycopg2-binary` は含まれていない（削除済み）
  - `src/` 内での使用箇所はコメント内のみ（実使用なし）
...
- ✅ **Step 0**: 依存方向の確定（循環参照の防止）
  - 依存関係グラフ生成完了
  - 循環依存なし（`services/ai.py` と `services/session.py` の相互依存なし）
  - 依存方向性ルール文書化完了
  - 不要な依存関係の特定完了（deptry を使用）

- ✅ **Step 1**: 足場固め
  - `config.py` の Pydantic V2 化完了
  - `utils/`, `errors/` の作成・移動完了
  - 対応テスト移行・実行完了

- ✅ **Step 2**: データ層とビジネスロジック
  - `db/models.py` の移動完了
  - `services/session.py` の作成完了
  - `SessionManager` に `is_initialized` プロパティ追加完了
  - 対応テスト移行・実行完了

- ✅ **Step 3**: AIサービスと抽象化
  - `services/ai.py` の統合完了
  - 例外ラッピング完了（`errors/ai.py` 作成）
  - API キー取得方法統一完了
  - 戻り値変更完了（`tuple[str, TokenInfo]`）
  - `TokenInfo` dataclass が `frozen=True, kw_only=True` で定義完了
  - `services/eavesdrop.py` の統合完了
  - リトライ処理が tenacity デコレータで実装完了

- ✅ **Step 4**: プレゼンテーション層（最大の山場）
  - `bot/handlers/` の物理分割完了（`__init__.py`, `mention.py`, `thread.py`, `eavesdrop.py`）
  - `setup_handlers` (DI) 実装完了
  - `TYPE_CHECKING` ガード使用完了
  - 循環参照回避完了
  - `bot/router.py` と `bot/commands.py` の移動完了
  - 対応テスト移行・実行完了

- ✅ **Step 5**: 結合と仕上げ
  - `main.py` の実装完了
  - Graceful Shutdown 実装完了
  - インポートパス最終確認完了
  - `__init__.py` 整備完了
...
- ✅ **Step 7**: 品質向上
  - 型ヒントの完全化（主要なハンドラー、サービスに型ヒント追加済み）
  - docstring の追加（主要な公開APIにdocstring追加済み）
...
3. **テストの充実**:
   - テスト構造がソースコード構造と対応完了
   - 全テスト通過（313個すべて成功、スキップ0個）
   - テストカバレッジ 70%（目標80%は未達だが、主要機能はカバー済み）
