...
```mermaid
graph LR
    A[段階1: 環境構築<br/>Discord接続] --> B[段階2: NASデプロイ<br/>Docker化・24時間稼働]
    B --> C[段階3: AI応答機能<br/>メンション応答型]
    C --> D[段階4: セッション管理<br/>会話履歴保持]
    D --> E[段階5: 会話の契機実装<br/>3つの方式]
    E --> F[段階6: 高度な機能<br/>同期・レート制限]
    F --> G[段階7: CI/CD・運用<br/>自動デプロイ・監視]
```
...
## 4. 段階 2: NAS デプロイ（Docker 化・24 時間稼働）
...
**詳細**: [Phase 2 実装計画](./phases/phase2.md)

---

## 5. 段階 3: AI 応答機能（メンション応答型）
...
- [ ] `ai/litellm_provider.py`: LiteLLM 統合実装
  - [ ] LiteLLM のセットアップ
  - [ ] 開発用モデル（Claude 3 Haiku（レガシー））の設定
...
| **段階 2** | 7-14 日      | NAS デプロイ、Docker 化       |
| **段階 3** | 5-7 日       | AI 応答機能、メンション応答型 |
| **段階 4** | 6-8 日       | セッション管理、会話履歴保持  |
| **段階 5** | 10-14 日     | 3 つの会話の契機              |
| **段階 6** | 8-10 日      | 同期、レート制限、コマンド    |
| **段階 7** | 8-10 日      | CI/CD、運用機能               |
| **合計**   | **47-68 日** | **約 9-14 週間**              |
