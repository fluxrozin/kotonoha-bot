...
| FR-007 | スレッド型               | メンション時に自動でスレッドを作成し、その中で会話を継続 | 高     | Phase 5      |
| FR-008 | 聞き耳型（LLM 判断）     | LLM に「空気」を読ませて、適切なタイミングで会話に参加   | 高     | Phase 5      |
| FR-009 | 聞き耳型（ルールベース） | ルールベースと確率で判断して会話に参加                   | 中     | Phase 5      |
...
| FR-014 | セッション保存 | SQLite に永続化                                  | 高     | Phase 1      |
| FR-015 | セッション復元 | ボット再起動時に SQLite から復元                 | 中     | Phase 1      |
| FR-016 | セッション削除 | 非アクティブセッションを削除                     | 中     | Phase 4      |
| FR-017 | セッション同期 | メモリと SQLite の同期                           | 高     | Phase 6      |

### 1.4 AI 機能

| ID     | 機能名           | 説明                       | 優先度 | 実装フェーズ |
| ------ | ---------------- | -------------------------- | ------ | ------------ |
| FR-018 | 基本応答生成     | Claude API で応答を生成    | 高     | Phase 1      |
| FR-019 | モデル使い分け   | タスクに応じてモデルを選択 | 高     | Phase 1      |
| FR-020 | プロンプト最適化 | 場面緘黙支援向けプロンプト | 高     | Phase 1      |
| FR-021 | 判定機能         | 聞き耳型の判定（Yes/No）   | 高     | Phase 5      |
| FR-022 | フォールバック   | API エラー時の代替処理     | 中     | Phase 1      |

### 1.5 エラーハンドリング機能

| ID     | 機能名           | 説明                             | 優先度 | 実装フェーズ |
| ------ | ---------------- | -------------------------------- | ------ | ------------ |
| FR-023 | API エラー処理   | Claude/Discord API エラー処理    | 高     | Phase 1      |
| FR-024 | レート制限対応   | レート制限の監視と対応           | 高     | Phase 6      |
| FR-025 | リトライロジック | エラー時の自動リトライ           | 高     | Phase 6      |
...
| ID     | 機能名         | 説明               | 優先度 | 実装フェーズ |
| ------ | -------------- | ------------------ | ------ | ------------ |
| FR-027 | `/chat start`  | スレッド型開始     | 高     | Phase 5      |
| FR-028 | `/chat reset`  | 会話履歴リセット   | 中     | Phase 5      |
| FR-029 | `/chat status` | セッション状態表示 | 低     | Phase 6      |
| FR-030 | `/settings`    | 設定の表示・変更   | 低     | Phase 7      |

### 1.7 運用機能

| ID     | 機能名         | 説明                           | 優先度 | 実装フェーズ |
| ------ | -------------- | ------------------------------ | ------ | ------------ |
| FR-031 | ログ出力       | 動作ログの記録                 | 中     | Phase 7      |
| FR-032 | モニタリング   | パフォーマンス監視             | 中     | Phase 7      |
| FR-033 | バックアップ   | データベースの自動バックアップ | 中     | Phase 7      |
| FR-034 | ヘルスチェック | システム状態の確認             | 中     | Phase 7      |
...
| ID      | 要件名              | 説明                                                              |
| ------- | ------------------- | ----------------------------------------------------------------- |
| NFR-016 | 低コスト API の使用 | 低コストで利用可能な API を使用（Claude 3 Haiku（レガシー）など） |
| NFR-017 | コスト最適化        | 開発・本番で適切なモデルを選択し、コストを最適化                  |
...
- FR-014: セッション保存（Phase 1 で実装済み）
- FR-015: セッション復元（Phase 1 で実装済み）
- FR-024: レート制限対応
- FR-027: `/chat start`
- NFR-012: API 障害対応
- NFR-015: 24 時間稼働（Phase 2 で実装予定）
