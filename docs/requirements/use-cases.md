...
| UC-002 | スレッド型で会話を開始する       | 高     | Phase 2      |
| UC-003 | DM 型で会話を開始する            | 中     | Phase 2      |
| UC-004 | 聞き耳型で自然に会話に参加する   | 高     | Phase 2      |
| UC-005 | 会話履歴をリセットする           | 中     | Phase 2      |
...
**概要**: ユーザーが Bot と 1 対 1 で DM で会話を開始します。

**アクター**: ユーザー、Kotonoha Bot

**前提条件**:

- Bot が DM を受け取る設定になっている
- ユーザーが Bot に DM を送信できる

**基本フロー**:

1. ユーザーが Bot に DM を送信（「こんにちは」）
2. Bot が DM を検知
3. Bot がセッションを作成（セッションキー: `dm:user_id`）
4. Bot が AI で応答を生成
5. Bot が DM で応答を送信
6. 会話履歴をセッションに保存
...
- 3a. 既存の DM セッションがある場合、そのセッションを使用

**事後条件**:

- DM セッションが作成され、会話履歴が保存されている
- ユーザーが Bot の応答を DM で受信している

**例外**:

- Bot が DM を受け取れない設定の場合: メッセージは無視される

---

### UC-004: 聞き耳型で自然に会話に参加する
...
4. Bot が Gemini Flash で判定（「今、発言すべきか？」→ YES/NO）
5. 判定が「YES」の場合:
   a. Bot が Gemini で応答を生成
...
graph TB
    ...
    User -->|DM送信| UC003[UC-003: DM型]
    User -->|メッセージ送信| UC004[UC-004: 聞き耳型]
