...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            postgresql.ENUM(
                "active", "archived", name="session_status_enum", create_type=False
            ),
    ...
    op.create_index(
        ...
        postgresql_where=sa.text("embedding IS NOT NULL AND token_count > 0"),
    )
    op.execute("""
