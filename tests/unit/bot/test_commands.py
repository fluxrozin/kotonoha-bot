...
from kotonoha_bot.bot.commands import ChatCommands
...
@pytest.mark.asyncio
async def test_chat_reset_dm(chat_commands, mock_handler):
    """/chat reset コマンド（DM型）"""
    # モックの設定
    interaction = MagicMock(spec=discord.Interaction)
    interaction.response = MagicMock()
...
@pytest.mark.asyncio
async def test_chat_reset_error(chat_commands, mock_handler):
    """/chat reset コマンド（エラー発生時）"""
    # モックの設定
    interaction = MagicMock(spec=discord.Interaction)
    interaction.response = MagicMock()
    interaction.response.defer = AsyncMock()
    ...
    # エラーを発生させる
    mock_handler.session_manager.get_session = AsyncMock(
    ...
    interaction.followup.send.assert_called_once()
    call_args = interaction.followup.send.call_args
    assert "失敗しました" in call_args[0][0]


@pytest.mark.asyncio
async def test_chat_status_dm(chat_commands, mock_handler):
    """/chat status コマンド（DM型）"""
    # モックの設定
    interaction = MagicMock(spec=discord.Interaction)
    interaction.response = MagicMock()
    interaction.response.defer = AsyncMock()
    ...
    mock_session.get_conversation_history = MagicMock(return_value=[])
    mock_session.created_at = datetime(2026, 1, 15, 10, 30, 0)
    # get_sessionを明示的にAsyncMockとして設定（警告を防ぐため）
    mock_handler.session_manager.get_session = AsyncMock(return_value=mock_session)
    ...
    # 検証
    call_args = interaction.followup.send.call_args
    assert "DM型" in call_args[0][0]


@pytest.mark.asyncio
async def test_chat_status_error(chat_commands, mock_handler):
    """/chat status コマンド（エラー発生時）"""
    # モックの設定
    interaction = MagicMock(spec=discord.Interaction)
    interaction.response = MagicMock()
    interaction.response.defer = AsyncMock()
